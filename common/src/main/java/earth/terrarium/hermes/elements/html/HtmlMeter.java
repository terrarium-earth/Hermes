...
public class HtmlMeter extends BasicElement<HermesStyle, HermesRenderer> implements Inline {
    ...
    public HtmlMeter(@NotNull HermesStyle style, @NotNull LayoutStyle layoutStyle, @Nullable Element<HermesStyle, HermesRenderer> parent, @NotNull String qName, @Nullable Attributes attributes) {
        super(style, layoutStyle, parent, qName, attributes);
        assert attributes != null;

        this.min = AttributeParser.parseFloat(attributes, "min", 0f);
        this.max = AttributeParser.parseFloat(attributes, "max", 1f);
        this.value = AttributeParser.parseFloat(attributes, "value", 0f);

        var low = Math.max(AttributeParser.parseFloat(attributes, "low", this.min), this.min);
        var high = Math.min(AttributeParser.parseFloat(attributes, "high", this.max), this.max);
        var optimum = AttributeParser.parseFloat(attributes, "optimum", Float.NaN);

        String defaultColor;
        if (Float.isNaN(optimum)) {
            defaultColor = "limegreen";
        } else if (optimum >= min && optimum <= low) {
            defaultColor = this.value <= optimum ? "limegreen" : "orange";
        } else if (optimum >= high && optimum <= max) {
            defaultColor = this.value >= optimum ? "limegreen" : "orange";
        } else {
            defaultColor = "gold";
        }
        ...
        this.barColor = ColorFactory.web(css.getOrDefault("color", defaultColor));
    }
    ...
    protected void drawElement(float x, float y, float width, float height, HermesRenderer renderer) {
        ...
        float percent = (this.value - this.min) / (this.max - this.min);
        float barWidth = width * Mth.clamp(percent, 0f, 1f);
