...
public class HtmlImage extends ImageElement<HtmlStyle, HtmlRenderer, CustomImage> {
    ...
    public HtmlImage(@NotNull HtmlStyle style, @NotNull LayoutStyle layoutStyle, @Nullable Element<HtmlStyle, HtmlRenderer> parent, @NotNull String qName, @Nullable Attributes attributes) {
    ...
    public static HtmlImage forEmbed(@NotNull HtmlStyle style, @NotNull LayoutStyle layoutStyle, @Nullable Element<HtmlStyle, HtmlRenderer> parent, @NotNull String qName, @NotNull Attributes attributes) {
    ...
    public static HtmlImage forObject(@NotNull HtmlStyle style, @NotNull LayoutStyle layoutStyle, @Nullable Element<HtmlStyle, HtmlRenderer> parent, @NotNull String qName, @NotNull Attributes attributes) {
    ...
    @Override
    public void drawInternal(float xOffset, float yOffset, float mouseX, float mouseY, HtmlRenderer renderData) {
        super.drawInternal(xOffset, yOffset, mouseX, mouseY, renderData);
        if (this.image == null) return;
        CursorScreen.Cursor cursor = this.layoutStyle.get(GlobalAttributesElement.CURSOR);
        if (cursor != null) {
            renderData.setCursor(cursor);
        } else {
            ImageMap imageMap = this.style.data().get(ImageMap.DATA_KEY, map);
